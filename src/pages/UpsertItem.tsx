              ...
              switch (f.type) {
                case "text":
                  return (
                    <div key={f._id} className="space-y-6">
                      <Label htmlFor={_id}>{f.long}</Label>
                      <Input id={_id} value={val ?? ""} onChange={(e) => setValue(f._id, e.target.value)} />
                    </div>
                  );
                case "number":
                  return (
                  ...
                  return (
                    <div key={f._id} className="space-y-6">
                      ...
                      <Select value={val ?? ""} onValueChange={(v) => setValue(f._id, v)}>
                        <SelectTrigger id={_id} className="w-[100%]"><SelectValue placeholder="Select…" /></SelectTrigger>
                        <SelectContent>
                          {(f.options || []).map((o, i) => (
                ...
                case "textarea":
                  return (
                    <div key={f._id} className="space-y-6">
                      <Label htmlFor={_id}>{f.long}</Label>
                      <Textarea id={_id} value={val ?? ""} onChange={(e) => setValue(f._id, e.target.value)} />
                    </div>
                  );
                default:
                  return null;
