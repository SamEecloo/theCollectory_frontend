...
export default function ComingSoon() {
  ...
  return (
    <div
      className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden bg-ink"
      style={{ fontFamily: "'Outfit', sans-serif" }}
    >
      {/* Ambient background glow */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-[-20%] left-[-10%] w-[600px] h-[600px] rounded-full opacity-20"
          style={{ background: 'radial-gradient(circle, #00421c, transparent 70%)' }} />
        <div className="absolute bottom-[-20%] right-[-10%] w-[500px] h-[500px] rounded-full opacity-15"
          style={{ background: 'radial-gradient(circle, #517662, transparent 70%)' }} />
      </div>
      ...
      {/* Gold top line */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#b8965a] to-transparent opacity-60" />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-xl w-full"
        ...
        {/* Logo */}
        <LogoWordmark className="h-10 w-auto fill-white mb-14 opacity-95" />

        {/* Badge */}
        <div className="flex items-center gap-3 mb-8">
          <div className="w-8 h-px bg-[#b8965a]" />
          <span className="text-[#b8965a] text-xs tracking-[0.25em] uppercase font-normal">Coming soon</span>
          <div className="w-8 h-px bg-[#b8965a]" />
        </div>

        {/* Headline */}
        <h1
          className="text-white font-light leading-none tracking-tight mb-6"
          style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(2.8rem, 8vw, 5.5rem)' }}
        >
          Every collection<br />
          has a <em className="italic text-[#a8c5b0]">story.</em>
        </h1>

        {/* Tagline */}
        <p
          className="text-white/40 font-light italic mb-14 tracking-wide"
          style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: 'clamp(1rem, 2vw, 1.35rem)' }}
        >
          Tell yours — launching soon.
        </p>
        {/* Already have account */}
        <button
          onClick={() => navigate('/login')}
          className="mt-8 text-white/25 hover:text-white/60 text-xs tracking-[0.15em] uppercase transition-colors duration-200"
        >
      ...
      <div className="absolute bottom-8 left-0 right-0 flex justify-center">
        <p className="text-white/15 text-xs tracking-[0.2em] uppercase">thecollectory.eu</p>
      </div>

      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,300;1,300&family=Outfit:wght@300;400;500&display=swap');
        @keyframes fadeUp {
